...
public class GameFrame extends Frame implements KeyListener, Runnable {
	...
	public void adjustAudio(){
		for(int i=0; i<objects.size(); i++) objects.get(i).adjust(px,py,pz,(double)degree);
	}
	...
	public void run(){
		while(true){
			...
			try { Thread.sleep(speed); } catch (InterruptedException e) { System.out.println("Sleep Failed."); }
		}
	}
}
